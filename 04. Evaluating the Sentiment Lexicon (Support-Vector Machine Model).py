...
for i in range(len(lemmas)):
    ...
    for j in range(len(ind)):
        ...
        obj += scores['Objective'][ind[j]]
    positive = pos / len(ind)
    negative = neg / len(ind)
    objective = obj / len(ind)
    sentiment[word] = np.array([positive, negative, objective])



#open Armenian glove, change entries to lemmas, and concatenate the sentiment scores
words = []
embedding_index = {}
...
total, count = 0, 0
embedding_matrix = np.zeros((vocab_size, 303))
...
training_matrix = np.zeros((len(X_train), maximum, 303))
testing_matrix = np.zeros((len(X_test), maximum, 303))

for i in range(len(X_train)):
    for j in range(maximum):
        training_matrix[i][j] = embedding_matrix[encoded_X_train[i][j]]
train = training_matrix.reshape(len(X_train), maximum * 303)

for i in range(len(X_test)):
    for j in range(maximum):
        testing_matrix[i][j] = embedding_matrix[encoded_X_test[i][j]]
test = testing_matrix.reshape(len(X_test), maximum * 303)
