...
#open the data
train = pd.read_csv('Train_Sentiment.csv', encoding = 'utf-8')
test = pd.read_csv('Test_Sentiment.csv', encoding = 'utf-8')
...
#open and prepare the lexicons as before
...
#associate sentiment score to each unique lemma
sentiment = {}
for i in range(len(scores)):
    word = scores['Lemma'][i]
    ind = np.where(scores['Lemma'] == word)[0]
    pos, neg = 0, 0
    for j in range(len(ind)):
        pos += scores['Positive'][ind[j]]
        neg += scores['Negative'][ind[j]]
    positive = pos / len(ind)
    negative = neg / len(ind)
    sentiment[word] = np.array([positive, negative])


# In[16]:


#open Armenian glove and concatenate the sentiment scores
words = []
embedding_index = {}
...
total, count = 0, 0
embedding_matrix = np.zeros((vocab_size, 302))
for word, i in t.word_index.items():
    total += 1
    embedding_vector = embedding_index.get(word)
    sentiment_vector = sentiment.get(word)
    if embedding_vector is not None:
        embedding_matrix[i][:300] = embedding_vector
    if sentiment_vector is not None:
        count += 1
        embedding_matrix[i][300:] = sentiment_vector



training_matrix = np.zeros((len(X_train), maximum, 302))
testing_matrix = np.zeros((len(X_test), maximum, 302))

for i in range(len(X_train)):
    for j in range(maximum):
        training_matrix[i][j] = embedding_matrix[encoded_X_train[i][j]]
train = training_matrix.reshape(len(X_train), maximum * 302)

for i in range(len(X_test)):
    for j in range(maximum):
        testing_matrix[i][j] = embedding_matrix[encoded_X_test[i][j]]
test = testing_matrix.reshape(len(X_test), maximum * 302)
...
def train_predict(clf):
    ...
    f1 = f1_score(list(y_test), y, pos_label = 'positive')
    rec = recall_score(list(y_test), y, pos_label = 'positive')
    pre = precision_score(list(y_test), y, pos_label = 'positive')
...
def train_predict_cv(clf):
    ...
    f1 = f1_score(y_test, y, pos_label = 'positive')
    rec = recall_score(y_test, y, pos_label = 'positive')
    pre = precision_score(y_test, y, pos_label = 'positive')
    return clf, y, acc, f1, rec, pre
